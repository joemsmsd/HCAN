...
parser.add_argument('--time_core_window_size', type=int, default=3, help='window size for time-core neighbor aggregation in HCAN models')
...
# HCAN
parser.add_argument('--n_semantic_cores', type=int, default=16, help='Number of semantic cores for HCAN')
parser.add_argument('--num_refinement_steps', type=int, default=2, help='Number of refinement steps for HCAN')
parser.add_argument('--use_global_core', type=lambda x: x.lower() == 'true', default=True, help='Whether to use global core in HCAN')
parser.add_argument('--lambda_orth', type=float, default=0.0, help='orthogonality regularization weight')
parser.add_argument('--hcan_lite', type=int, default=1, help='use lite HCAN (reduced attention paths)')
parser.add_argument('--kv_dim', type=int, default=64, help='low-rank dim for K/V projections in attention')
parser.add_argument('--time_pool_topk', type=int, default=16, help='top-k neighbors for time soft pooling')
parser.add_argument('--share_params', type=int, default=1, help='share core/cross attention params across layers')
parser.add_argument('--time_normalize', type=int, default=1, help='min-max normalize time marks to [0,1]')

# HCAN_MultiQuery
parser.add_argument('--n_time_queries', type=int, default=5, help='Number of independent time queries for HCAN_MultiQuery')
