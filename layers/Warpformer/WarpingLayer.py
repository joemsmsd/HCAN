...
class Almtx(nn.Module):
    ...
    # Use linspace to get exactly S points in [0, 1 - 1/S]
        rl_vals = torch.linspace(0.0, 1.0 - 1.0/S, steps=S, device=gamma.device, dtype=gamma.dtype)
        new_Rl = torch.min(vl, rl_vals).unsqueeze(-1).expand(B,S,L)
    ...
    # Use linspace to get exactly S points in [1/S, 1]
        rr_vals = torch.linspace(1.0/S, 1.0, steps=S, device=gamma.device, dtype=gamma.dtype)
        tmp_Rr = torch.max(vr, rr_vals).unsqueeze(-1).expand(B,S,L)
        new_Rr = tmp_Rr.clone()
    ...
    # Build boundary grids with exact S length to avoid floating point arange drift
        Rl_vals = torch.linspace(0.0, 1.0 - 1.0/self.S, steps=self.S, device=gamma.device, dtype=gamma.dtype)
        Rr_vals = torch.linspace(1.0/self.S, 1.0, steps=self.S, device=gamma.device, dtype=gamma.dtype)
        Rl = Rl_vals.unsqueeze(0).unsqueeze(-1).expand(1, self.S, L)
        Rr = Rr_vals.unsqueeze(0).unsqueeze(-1).expand(1, self.S, L)
