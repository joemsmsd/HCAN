...
class LatentODE(VAE_Baseline):
    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, 
        ...
        first_point_std = first_point_std.abs() + 1e-6
        ...
        if self.use_poisson_proc:
            ...
            # Check for numerical stability (changed from assert to warning)
            if torch.sum(int_lambda[:,:,0,:]) != 0.:
                print(f"Warning: int_lambda at t=0 is not zero: {torch.sum(int_lambda[:,:,0,:])}")
            if torch.sum(int_lambda[0,0,-1,:] <= 0) > 0:
                print(f"Warning: int_lambda has non-positive values: {torch.sum(int_lambda[0,0,-1,:] <= 0)} elements")
                # Clamp to small positive value to avoid numerical issues
                int_lambda = torch.clamp(int_lambda, min=1e-10)
