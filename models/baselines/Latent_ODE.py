...
class Model(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        x_mark_fixed = mark[:x.shape[1]].unsqueeze(0).unsqueeze(-1).expand(BATCH_SIZE, -1, 1)
        y_mark_fixed = mark[x.shape[1]: x.shape[1] + y.shape[1]].unsqueeze(0).unsqueeze(-1).expand(BATCH_SIZE, -1, 1)

        pred, loss = self.model(
            batch_dict={
                "tp_to_predict": y_mark_fixed[0, :, 0],
                "observed_data": x,
                "observed_tp": x_mark_fixed[0, :, 0],
