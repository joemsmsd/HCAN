...
class Model(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        # sanity check: ensure the configured enc_in matches the input's channel count
        if ENC_IN != getattr(self, 'enc_in', None):
            raise ValueError(
                f"configs.enc_in ({getattr(self, 'enc_in', None)}) != input channels ENC_IN ({ENC_IN}). "
                "If configs.enc_in < ENC_IN the model will index out of bounds during gather. "
                f"Set --enc_in to {ENC_IN} (or fix the dataset preprocessing) and re-run."
            )
