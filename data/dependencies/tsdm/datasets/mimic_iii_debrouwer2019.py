...
class MIMIC_III_DeBrouwer2019(MultiFrameDataset):
    ...
    def _clean(self, key):
        ...
        if ts.shape != self.RAWDATA_SHAPE:
            print(f"WARNING: Data shape {ts.shape} differs from expected {self.RAWDATA_SHAPE}")
            print("Continuing execution with current data. Results may be inconsistent.")
            print("For proper results, use bin_k=2 preprocessing from https://github.com/edebrouwer/gru_ode_bayes/")
            # raise ValueError(
